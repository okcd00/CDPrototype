...
from solver.losses import LOSS_FN_LIB
from engine.model_example import CscTrainingModel
from data.loaders.collator import DataCollatorForCsc
...
class CDMacBertForCsc(CscTrainingModel):
    ...
    def forward(self, texts, cor_labels=None, det_labels=None, 
        ...
        if cor_labels is None:
            ...
            outputs = (det_prob, cor_prob, pinyin_prob)
            return outputs
...
def model_test(text, method='eval'):
    dir_path = '/data/chendian/cleaned_findoc_samples/checkpoints/cdmac_autodoc_220705/'
    ckpt_file = dir_path + '/epoch=02_train_loss_epoch=0.4846_train_det_f1_epoch=0.8001_train_f1_epoch=0.0024.ckpt'
...
if __name__ == "__main__":
    ...
    model_test(text='杨博说要去乡下探亲访友', method='train')
