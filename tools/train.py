...
# data
from data.build import make_loaders
from data.loaders import get_csc_loader, DataCollatorForCsc, DynamicDataCollatorForCsc
...
def build_model():
    ...
    model = CDMacBertForCsc(cfg, tokenizer)
...
def main():
    ...
    if global_rank == 0:
        ...
        from utils.evaluations import eval_sighan2015_by_model
...
def main_last(save_per_n_steps=0):
    ...
    if save_per_n_steps > 0:
        ...
        ckpt_callback = ModelCheckpoint(
            ...
            dirpath=get_abs_path(cfg.OUTPUT_DIR),
            filename='{epoch:02d}_{train_loss_epoch:.4f}_{train_det_f1_epoch:.4f}_{train_f1_epoch:.4f}',
            save_top_k=3,
            mode='min'
        )
